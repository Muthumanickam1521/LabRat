%--------------------------------------------------------
% 
% ctrl + shift + scrollmouse = zoom(in or out)
%
...
dt_B = zeros(1, NP);                    % A range from 0 to NP
...
for i = 1: NP
    ...
    dt_B(i) = dt_Our*sqrt(1 - (v(i + 1)/c)^2)^0.5;                                  %[in s]
    
    
    x_T(i + 1) = x_T(i) + v(i + 1)*dt_B(i);                                         %[in m]
    
    
    r_BT(i + 1) = sqrt((x_B - x_T(i + 1))^2 + (y_B - y_T)^2 + (z_B - z_T)^2);       %[in m]
   
    
    Fg(i) = G*m_B*m_T/(r_BT(i + 1))^2 + m_T*(v(i + 1) - v(i))/dt_B(i);              %[in N]                              
    
    
    t(i + 1) = t(i) + dt_B(i);                                                      %[in s]

end


%Plotting
plot(t(1: 100), Fg(1: 100), 'co-');    %plotting X and Y
